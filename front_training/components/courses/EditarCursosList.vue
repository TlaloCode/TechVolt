<template>
    <div class="py-8 max-w-full bg-white text-gray-700 shadow">
        <div class="max-w-screen-xl mx-auto">
            <div class="w-full">
                <div class="flex container md:px-8">
                    ...
                    <div class="flex flex-initial justify-end content-center">
                        <NuxtLink
                ...
                <div class="flex container md:px-8">
                    <p class="md:px-5 mt-5 flex items-center text-xl flex-auto">
                        <span v-if="coursesList.length === 0">Agrega un curso</span>
                        <span v-else>Elija un curso a editar</span>
                    </p>
                </div>
            </div>
                        ...
                        >
                            <div class="px-4 py-4 sm:px-6">
                                <div class="flex items-center">
                                    ...
                                    >
                                        <span class="tooltip-box"> Eliminar curso </span>
                                        <i class="bx bx-trash text-xl"></i>
                                    </button>
                                    <NuxtLink
                                        :to="`/gestion/mis-cursos/${course.id}/editar`"
                                    ...
                                    >
                                        <span class="tooltip-box"> Editar curso </span>
                                        <i class="bx bx-edit-alt text-xl"></i>
                                    </NuxtLink>
                                    <NuxtLink
                                        :to="`/gestion/mis-cursos/${course.id}/modulos/nuevo`"
                                        class="border hover:bg-blue-600 hover:text-white focus:ring-4 focus:outline-none focus:ring-blue-200 font-medium rounded-full text-sm p-2 text-center inline-flex items-center dark:border-blue-400 dark:text-blue-400 dark:hover:text-white dark:focus:ring-blue-600 relative tooltip mx-2"
                                    >
                                        <span class="tooltip-box"> Agregar módulo </span>
                                        <i class="bx bx-layer-plus text-xl"></i>
                                    </NuxtLink>
                                    <NuxtLink
                                        :to="`/gestion/mis-cursos/${course.id}/modulos`"
                                        class="border hover:bg-blue-600 hover:text-white focus:ring-4 focus:outline-none focus:ring-blue-200 font-medium rounded-full text-sm p-2 text-center inline-flex items-center dark:border-blue-400 dark:text-blue-400 dark:hover:text-white dark:focus:ring-blue-600 relative tooltip mx-2"
                                    >
                                        <span class="tooltip-box"> Módulos </span>
        ...
        <vs-dialog v-model="isOpenDialogDeleteCourse">
            <template #header>
                <h4 class="m-1 font-medium">Confirmar acción</h4>
            </template>

            <div class="text-center">
                <p class="mb-3">¿Está seguro que desea eliminar este curso?</p>
...
export default {
    ...
    data() {
        return {
            ...
            isOpenDialogDeleteCourse: false,
            coursesList: [],
            dataReturned: '',
        }
    },
    ...
    methods: {
        async getCourse() {
            ...
            try {
                this.dataReturned = await this.$store.dispatch('course/getCourseList', params)
                this.coursesList = this.dataReturned.results
            } catch (err) {
                let res = ''
                if (err.response) {
        ...
        deleteCourse(courseId) {
            ...
            this.isOpenDialogDeleteCourse = true
        },
        async submitDeleteCourse() {
            ...
            try {
                await this.$store.dispatch('course/deleteCourse', this.courseToDeleteId)
                this.$vs.notification({
                    color: 'success',
                    title: 'Éxito',
                ...
                this.isOpenDialogDeleteCourse = false
                loading.close()
            }
        },
    },
